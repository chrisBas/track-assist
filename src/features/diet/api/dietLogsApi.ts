...
export function getDietLogs(date: string) {
  ...
  const query =
    'id, created_by, datetime, servings, foods( id, name, unit_qty, calories, created_by, units-of-measurement( id, name, abbreviation, created_by ) )';
  ...
  return supabase
    ...
    .then((response) => response.data as DietLogItemExpanded[]);
}
